...
def get_scores(Y_test, y_class, y_prob, y_train):
...
def get_predictions(X_train, Y_train, X_test, Y_test):
    ...
    log_scores = get_scores(Y_test, Y_log_pred_class, Y_log_pred_proba, Y_train)
    ...
    lgbm_scores = get_scores(Y_test, Y_lgbm_pred_class, Y_lgbm_pred_proba, Y_train)

    random_forest = RandomForestClassifier(n_estimators=100)
    random_forest.fit(X_train, Y_train)
    Y_rf_pred_class = random_forest.predict(X_test)
    Y_rf_pred_proba = random_forest.predict_proba(X_test)[:,1]
    rf_scores = get_scores(Y_test, Y_rf_pred_class, Y_rf_pred_proba, Y_train)
