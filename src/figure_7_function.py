...
def plot_ffc_reeval(eval_df, figure_path):
    """Make the figure!"""
    ...
    legend_elements = [Line2D([0], [0], markersize=8, marker='o',
                              markerfacecolor=([62/255, 138/255, 187/255, 0.35]),
                              markeredgecolor=([62/255, 138/255, 187/255, 1]),
                              label=r'w$_0$ = $\bar{y}_{\mathrm{Train}}$', linestyle='none'),
                       Line2D([0], [0], markersize=8, marker='o',
                              markerfacecolor=([244/255, 109/255, 67/255, 0.35]),
    ...
    ax1.text(0.175, 0.89, r'$\widebar{y}_{\mathrm{Train}}$ = 0.209', fontsize=13, ha='center', va='center', transform=ax1.transAxes)
    ax2.text(0.175, 0.89, r'$\widebar{y}{_\mathrm{Train}}$ = 0.235', fontsize=13, ha='center', va='center', transform=ax2.transAxes)
    ax3.text(0.175, 0.89, r'$\widebar{y}_{\mathrm{Train}}$ = 0.060', fontsize=13, ha='center', va='center', transform=ax3.transAxes)
...
def build_df(prob_subm):
    ...
    for outcome in prob_subm['outcome'].unique():
        for account in prob_subm[prob_subm['outcome'] == outcome]['account'].sort_values(ascending=False).unique():
        ...
        outcome_positive = outcome_df[outcome_df['r2_sub'].astype(float)>0]
        outcome_positive['incremental'] = outcome_positive['r2_sub'] - outcome_positive['r2_LPM']
        print('We have ' + str(len(outcome_df)) + ' submissions for ' + outcome)
        print('We have ' + str(len(outcome_df[outcome_df['r2_sub'] > 0])) + ' submissions for ' +
              outcome + ' which have an r2>0')
        print('The min E(w) for ' + outcome + ' vs prev is ' + str(outcome_df['ew_sub_vs_prev'].min()))
        print('The min E(w) for ' + outcome + ' vs benchmark is ' + str(outcome_df['ew_sub_vs_LPM'].min()))
        print('The max E(w) for ' + outcome + ' vs prev is ' + str(outcome_df['ew_sub_vs_prev'].max()))
        print('The max E(w) for ' + outcome + ' vs benchmark is ' + str(outcome_df['ew_sub_vs_LPM'].max()))
        print('Spearmans Rho for incremental r2 and ew for ' + outcome + ': ' +
              str(outcome_positive['incremental'].astype('float64').corr(outcome_positive['ew_sub_vs_LPM'].astype('float64'))))
    return eval_df
...
def clean_subm(subm):
    ...
    for account in temp_subm['account'].unique():
        for outcome in temp_subm['outcome'].unique():
            temp = temp_subm[(temp_subm['outcome'] == outcome) & (temp_subm['account'] == account)]
...
def clean_prob_subm(subm):
    ...
    for account in subm['account'].unique():
        for outcome in subm['outcome'].unique():
            temp = subm[(subm['outcome'] == outcome) & (subm['account'] == account)]
            if (len(temp[(temp['prediction'] > 0) & (temp['prediction'] < 1)]) > 0) and \
...
def load_data(data_dir):
    """Load the FFC data"""
    subm = pd.read_csv(os.path.join(data_dir, 'submissions.zip'),
                       compression='zip', low_memory=False)
    bench = pd.read_csv(os.path.join(data_dir, 'intermediate_files',
                                     'benchmarks_long.zip'),
                        compression='zip', low_memory=False)
    return subm, bench
...
def preprocess_data(subm, bench):
    ...
    print('Check max IDs for any outcome (1104): ', len(bench['challengeID'].unique()))
    ...
    prob_subm = pd.merge(prob_subm, bench[['outcome_name', 'challengeID', 'prediction_LPM']],
                         how='left', left_on=['outcome_name', 'challengeID'],
                         right_on=['outcome_name', 'challengeID'])
    subm = pd.merge(subm, bench[['outcome_name', 'challengeID', 'prediction_LPM']],
                    how='left', left_on=['outcome_name', 'challengeID'],
                    right_on=['outcome_name', 'challengeID'])
...
def make_figure_seven(figure_path, data_dir):
    ...
    plot_ffc_reeval(eval_df, figure_path)